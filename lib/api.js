...
export const premiumAPI = {
  ...
  /** Admin: payouts in flight — approval first (D-028), then resolution of what M-Pesa didn't confirm (D-026). */
  async listPayoutReviews() {
    return apiRequest("/v2/payouts/review");
  },
  async approvePayout(txId, payload = {}) {
    return apiRequest(`/v2/payouts/${encodeURIComponent(txId)}/approve`, { method: "POST", body: JSON.stringify(payload) });
  },
  async declinePayout(txId, payload = {}) {
    return apiRequest(`/v2/payouts/${encodeURIComponent(txId)}/decline`, { method: "POST", body: JSON.stringify(payload) });
  },
  ...
  /** Admin: what the provider sent us about one payout (D-028). */
  async payoutCallbacks(txId) {
    return apiRequest(`/v2/payouts/${encodeURIComponent(txId)}/callbacks`);
  },
  /** Admin: the most recent provider callbacks across all transactions (D-028). */
  async listCallbacks({ source, limit = 50 } = {}) {
    const q = new URLSearchParams({ limit: String(limit), ...(source ? { source } : {}) });
    return apiRequest(`/v2/callbacks?${q}`);
  },
...
export const usersAPI = {
  ...
  /** KB 28 M1 — verification email through our pipeline; `data.fallback` means use Firebase's sender. */
  async sendVerificationEmail(continueUrl) {
    return apiRequest("/v2/users/me/verification-email", {
      method: "POST",
