...
const isoOf = (ts) => toDate(ts)?.toISOString() ?? null;
...
export async function getAdminSummary() {
  ...
  return {
    ...
    // cursor for the 6th onwards: the 5th event's timestamp
    latestCursor: audit?.events?.[4]?.at ? isoOf(audit.events[4].at) : null,
  };
}
