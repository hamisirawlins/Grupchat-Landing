...
- [x] M0 Resend domain verified: `mailing.grupchat.net` (2026-09-10); sender `GrupChat <hello@mailing.grupchat.net>`
- [x] M1 Email verification: link generated by `POST /v2/users/me/verification-email` and sent via Resend (Firebase sender = fallback); `/verify-email` page; Home banner; **withdrawals gated** on `email_verified` (server + sheet)
...
## Log (newest first)
- 2026-09-13 · Approving a payout now takes two taps and must name the amount (D-030) after a live test sent money on a single tap.
