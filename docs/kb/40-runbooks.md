...
## Email
`cd gc-payments && npm run email:domain` — probes Resend by sending to its sink address (the key is send-only); exit 0 = domain verified (verified for `mailing.grupchat.net` on 2026-09-10). Run occasionally while DNS propagates. `npm run email:test you@example.com` — sends the shared layout to one address. Failed sends are in the audit trail as `email.failed` with the Resend reason. Verification mail goes through the same pipeline (`verify` type, one per user per minute) and shows in the Resend dashboard and the audit trail; Firebase's own mailer (`noreply@<project>.firebaseapp.com`) is used only when Resend refuses the send.
