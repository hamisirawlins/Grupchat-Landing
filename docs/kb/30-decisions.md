...
**Consequences.** `/v2/mpesa/b2c-result`, `/v2/mpesa/b2c-timeout`; the V1 `withdrawal_callback`/`queue_timeout` routes bridge on `OriginatorConversationID = WITHDRAW_<txId>`. Plan details shows "on hold" and an owner-only Withdraw sheet that awaits confirmation (D-023).
