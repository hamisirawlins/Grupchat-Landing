...
export default function PoolsPage() {
  ...
  const loadPoolsData = async () => {
    try {
      ...
      const transformedPools = poolsData.map(pool => ({
        ...
        role: pool.memberships && pool.memberships.length > 0 ? pool.memberships[0].role : 'member',
      ...
      // Calculate stats
      const activeCount = transformedPools.filter(p => p.status === 'active').length;
      const completedCount = transformedPools.filter(p => p.status === 'completed').length;
      const totalAmount = transformedPools.reduce((sum, pool) => sum + pool.currentBalance, 0);
  ...
  const StatusBadge = ({ status }) => {
    ...
    return (
      <span className={`px-2.5 py-1 rounded-md text-xs font-medium border ${colors[status]}`}>
  ...
  const TypeIcon = ({ type }) => {
    ...
    return (
      <div className={`w-10 h-10 ${color} rounded-xl flex items-center justify-center text-white text-sm font-semibold shadow-md hover:shadow-lg transition-all duration-300 transform hover:scale-105`}>
        {emoji}
      </div>
    );
  };
    ...
    >
      ...
      <div className="space-y-4 mb-6">
        ...
        <div className="flex items-center justify-between">
          ...
          <span className="text-2xl font-bold text-gray-900">{pool.percentage}%</span>
        </div>
    ...
    >
        ...
        <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-6">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-sm text-gray-500">Track and manage all your pools</p>
            </div>
          </div>
          <button
        ...
        <div className="bg-white/80 backdrop-blur-xl rounded-2xl p-6 border border-white/20 shadow-sm hover:shadow-lg transition-all duration-300 group">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-2xl font-bold text-gray-900">KSh {stats.totalAmount.toLocaleString()}</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white/80 backdrop-blur-xl rounded-2xl border border-white/20 shadow-sm p-6 mb-6 hover:shadow-lg transition-all duration-300">
        <div className="flex flex-col lg:flex-row gap-4">
          ...
          <div className="flex flex-col sm:flex-row gap-4">
                  ...
                  <div className="absolute right-0 top-full mt-2 w-64 bg-white border border-gray-200 rounded-lg shadow-lg z-50">
                    <div className="p-4">
                      <div className="flex items-center justify-between mb-4">
                        <h3 className="font-medium text-gray-900">Sort pools by</h3>
                        <button
                          onClick={() => setShowSortDialog(false)}
                      ...
                      <div className="mt-4 pt-4 border-t border-gray-100">
                        <p className="text-xs text-gray-600">
                          Click the same field again to toggle between ascending and descending order.
                        </p>
                      </div>
                    </div>
                  </div>
            ...
            <div className="hidden md:flex border border-gray-200 rounded-xl overflow-hidden shadow-sm">
              <button
      ...
      {filteredPools.length === 0 ? (
        ...
        <>
          ...
          <div className={`grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-6 ${viewMode === 'table' ? 'md:hidden' : ''}`}>
