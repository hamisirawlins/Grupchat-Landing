...
export default function Dashboard() {
  ...
  const loadDashboardData = async () => {
    try {
      ...
      setUserPools(pools.slice(0, 6).map(pool => ({
        ...
        percentage: pool.target_amount > 0 ? Math.round((parseFloat(pool.current_balance) / parseFloat(pool.target_amount)) * 100) : 0,
  ...
  const MetricCard = ({ title, value, icon: Icon }) => (
    <div className="bg-white rounded-2xl p-6 border border-gray-100 relative group hover:shadow-lg transition-all duration-300">
  ...
  const TypeIcon = ({ type }) => {
    ...
    return (
      <div className={`w-10 h-10 sm:w-12 sm:h-12 ${color} rounded-xl flex items-center justify-center text-white text-sm sm:text-base font-semibold shadow-md hover:shadow-lg transition-all duration-300 transform hover:scale-105 flex-shrink-0`}>
        {emoji}
      </div>
    );
  };
  ...
  const PoolCard = ({ pool }) => (
    <div 
      ...
      className="bg-white rounded-2xl p-4 sm:p-6 border border-gray-100 hover:shadow-lg transition-all duration-300 cursor-pointer"
    >
      <div className="flex items-start justify-between mb-4 sm:mb-6">
        <div className="flex items-center gap-3 sm:gap-4 flex-1 min-w-0">
          <TypeIcon type={pool.type} />
          <div className="min-w-0">
            <h3 className="font-semibold text-gray-900 text-base sm:text-lg truncate">{pool.name}</h3>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 gap-4 sm:gap-6 text-sm mb-4 sm:mb-6">
        <div>
          <p className="text-gray-500 mb-1">Target</p>
          <p className="font-medium text-gray-900 text-xs sm:text-sm">KSh {pool.targetAmount.toLocaleString()}</p>
        </div>
        <div>
          <p className="text-gray-500 mb-1">Pooled</p>
          <p className="font-medium text-gray-900 text-xs sm:text-sm">KSh {pool.currentBalance.toLocaleString()}</p>
        </div>
        <div className="col-span-2 sm:col-span-1">
          <p className="text-gray-500 mb-1">Due Date</p>
          <p className="font-medium text-gray-900 text-xs sm:text-sm">{pool.dueDate}</p>
        </div>
      </div>
      ...
      <div className="mt-4">
        <div className="flex justify-between items-center mb-2">
          ...
          <span className="text-sm font-medium text-gray-900">{pool.percentage}%</span>
        </div>
                  ...
                  {item.children.map((child) => (
                    ...
                    >
                      <div className={`w-2 h-2 rounded-full ${
                        ...
                        child.status === 'done' ? 'bg-green-500' : 'bg-red-500'
                      }`} />
      ...
      <div className="flex-1 lg:ml-80">
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-4">
              ...
              <div className="hidden sm:block">
                <h1 className="text-lg lg:text-xl font-semibold text-gray-900">Hey, {user.displayName || 'Markus'}</h1>
        ...
        <main className="p-4 sm:p-6 lg:p-8 space-y-6 lg:space-y-8">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 lg:gap-6">
            <MetricCard title="Active Pools" value={dashboardData.metrics.activePools} icon={FolderOpen} />
            <MetricCard title="Total Members" value={dashboardData.metrics.totalMembers} icon={Users} />
            <MetricCard title="Total Transactions" value={dashboardData.metrics.totalTransactions} icon={BarChart} />
            <MetricCard title="Total Pooled (KSh)" value={dashboardData.metrics.totalPooled.toLocaleString()} icon={TrendingUp} />
          </div>
          ...
          <div className="grid grid-cols-1 xl:grid-cols-3 gap-6 lg:gap-8">
            ...
            <div className="xl:col-span-2">
              <div className="flex items-center justify-between mb-6">
                <div className="flex items-center gap-3">
                  ...
                  <h2 className="text-xl font-semibold text-gray-900">Active Pools</h2>
                </div>
                <button 
                  onClick={() => router.push('/create-pool')}
                  className="bg-blue-600 text-white px-3 sm:px-4 py-2 rounded-lg text-sm font-medium hover:bg-blue-700 transition-colors flex items-center gap-2"
                >
              ...
              <div className="space-y-4">
                {userPools.length > 0 ? (
                  ...
                  <div className="text-center py-12">
                    ...
                    <p className="text-gray-500 mb-6">Create your first pool to get started</p>
                    <button
                      onClick={() => router.push('/create-pool')}
            ...
            <div className="space-y-6 lg:space-y-8">
              ...
              <div className="bg-white rounded-2xl p-4 sm:p-6 border border-gray-100 shadow-sm">
                <div className="flex items-center justify-between mb-6">
                  <div className="flex items-center gap-3">
                    ...
                    <h3 className="font-semibold text-gray-900">Pool Completion</h3>
                  </div>
                </div>
              ...
              <div className="bg-white rounded-2xl p-4 sm:p-6 border border-gray-100 shadow-sm">
                <div className="flex items-center justify-between mb-6">
                  <div className="flex items-center gap-3">
                    ...
                    <h3 className="font-semibold text-gray-900">Transaction Trends</h3>
                  </div>
                ...
                <p className="text-sm text-gray-500 mb-4">Pool transactions and contributions over time.</p>
                <LineChartComponent data={chartData.performanceMetrics} height={180} />
              </div>
              ...
              <div className="bg-white rounded-2xl p-4 sm:p-6 border border-gray-100 shadow-sm">
                ...
                <p className="text-sm text-gray-500 mb-6">Most active pool contributors.</p>
            ...
            <div className="space-y-4">
              <div>
                ...
                <input
                  ...
                  className="w-full px-4 py-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent placeholder-gray-500 text-gray-900"
